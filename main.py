...
frame1 = Frame(root)
frame1.pack()
titleLabel = Label(frame1 , text="CourseWork" , font=("Arial" , 26) , bg="red" , width=16 )
...
def restartGame():
    ...
    titleLabel = Label(frame1 , text="CourseWork" , font=("Arial" , 30) , bg="red" , width=15 )
    titleLabel.grid(row=0 , column=0)
...
def play(event):
    ...
    if button["text"] == " ":
        if turn == "x" :
            board[clicked] = turn
            if checkForWin(turn):
                winningLabel = Label(frame1 , text=f"{turn} wins the game", bg="red", font=("Arial" , 26),width=16 )
